...
## 2024.02.21: 실습 페이지 코드 수정, MySQL 데이터 연동 및 Admin 페이지 작업
1. 작업 내용
  - 실습 페이지의 각 페이지 별 제품 목록(제품 명, 금액, 리뷰 수) MySQL 데이터 연동
  - Admin 페이지 구성

<br>

2. 문제점  
  - 깃허브 연결 이미지가 계속 깨져서 나오고 있음  
    -> 해당 코드는 현재는 마이너한 구성이기에, 우선 삭제 후, 최종 수정 시 추가 및 조치하기로 함

<br>

3. 이후 계획
  - 부트스트랩을 통해 Admin 페이지의 전체적인 구성 수정
  - 페이지네이션, 검색 기능 등 활성화 방법 확인 및 코드 반영

## 2024.02.08: 실습 페이지 탐구, 크롤링 코드 작성

1. 작업 내용
  - ~~알라딘 베스트셀러 1위~200위 스크래핑 코드 작성~~
  - 바늘이야기 홈페이지 [판매제품>실] 목록 스크래핑 코드 작성
...
2. 문제점  
  ...
  - 스크래핑 시, 결과값에 div태그가 함께 표시되는 오류 발생
    <img width="629" alt="코드 오류 발생" src="https://github.com/Ina-Youn/oz_class/assets/155051602/df7220f8-807e-49e9-a9c0-821123c11e85">  

    -> ':'을 기준으로 문자열을 나누어 ':'뒷 부분만 나오도록 코드 1차 수정 (리뷰 숫자만 필요하기 때문에 앞부분의 '리뷰: '는 표함하지 않음)  
    ```
    1차 수정 코드
    review = str(i.select_one(".pr_review")).split(' : ')[-1]
    ```  
    <img width="622" alt="코드 1차 수정" src="https://github.com/Ina-Youn/oz_class/assets/155051602/6ba9843d-708e-4b63-bb99-e77c600d500e">

  - '/div'태그가 계속해서 함께 표시되는 요류 발생  
   -> '<'를 기준으로 태그를 한번 더 나누어 숫자만 나오도록 코드 2차 수정
     ```
     # 2차 수정 코드
